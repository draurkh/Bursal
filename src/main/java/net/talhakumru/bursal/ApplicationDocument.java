...
public class ApplicationDocument {
	...
	public void validateCV(FacesContext context, UIComponent component, Object value) {
		...
		if (!file.getContentType().split("/", 2)[0].equals("image") && !file.getContentType().equals("application/pdf")) {
			// file type is incorrect
			
			((UIInput) component).setValid(false);
			
			FacesMessage typeError = new FacesMessage(file.getSubmittedFileName() + " dosya tipi kabul edilemez. Lütfen PDF veya bir görsel dosyası yükleyin. (.pdf veya .png, .jpg gibi)");
		...
		if (sizeInMBs >= CV_FILE_SIZE_LIMIT) {
			...
			FacesMessage sizeError = new FacesMessage(file.getSubmittedFileName() + " dosyasının boyutu çok büyük. Lütfen 5 Megabayt'tan (MB) daha küçük bir dosya yükleyin.");
	...
	private void saveFile() {
		...
		try(InputStream input = cvFile.getInputStream()) {
			...
			cvFileName = new CVFileName().get(firstName, lastName, fileExtension);	// creates file names with global counter
	...
	public String downloadCV() {
		...
		exContext.setResponseHeader("Content-Disposition", "attachment; filename=\"" + file.getName() + "\"");	// enables downloading attached file
	...
	public Document appToDoc() {
		...
		return new Document().append("firstName", this.getFirstName()) 
				.append("lastName", this.getLastName())
				.append("birthday", this.getBirthday()) 
				.append("university", this.getUniversity()) 
				.append("address", this.getAddress())
				.append("state", this.state)
				.append("cv_file_name", cvFileName);
	}
